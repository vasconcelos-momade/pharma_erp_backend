...
import { GenerateInvoiceArtifactsUseCase } from "../../modules/documents/application/use-cases/generate-invoice-artifacts.use-case";
import { runWithTenantBranchContext } from "../../shared/http/tenant-auth";
import {
  type GenerateInvoiceArtifactsJobPayload,
  JobQueueService,
...
async function handleGenerateInvoiceArtifactsJob(job: QueueJob<"document.generate-invoice-artifacts">) {
  const payload = job.payload as GenerateInvoiceArtifactsJobPayload;
  await runWithTenantBranchContext(payload.tenantId, payload.branchId, async () => {
    const useCase = new GenerateInvoiceArtifactsUseCase();
    await useCase.execute(payload.faturaId, payload.snapshotId);
    return Response.json({ ok: true });
  });
  console.log("[worker] document.generate-invoice-artifacts concluido:", {
    jobId: job.id,
...
async function handleJob(job: QueueJob) {
  ...
  switch (job.type) {
    ...
    case "billing.generate-monthly":
      ...
      return;
    case "document.generate-invoice-artifacts":
      await handleGenerateInvoiceArtifactsJob(job as QueueJob<"document.generate-invoice-artifacts">);
      return;
    default:
