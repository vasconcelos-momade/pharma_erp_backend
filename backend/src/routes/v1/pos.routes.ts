...
const invoiceDetailParamSchema = z.object({
  saleId: z.string().trim().regex(/^\d+$/, "saleId inválido"),
});
...
export function registerPosRoutes(router: Router, prefix: string): void {
  withTenantPos(router, "get", `${prefix}/tenant/pos/produtos/catalog-version`, async () =>
    posController.getCatalogVersion(),
  );
  withTenantPos(router, "get", `${prefix}/tenant/pos/products/catalog-version`, async () =>
    posController.getCatalogVersion(),
  );
  ...
  withTenantPos(router, "get", `${prefix}/tenant/pos/sales`, async (_userId, context) =>
    posController.listFaturas(context.req),
  );
  withTenantPos(router, "get", `${prefix}/tenant/pos/faturas/:saleId`, async (userId, context) =>
    posController.getFaturaDetalhe(
      parseRouteParams<{ saleId: string }>(context.params, invoiceDetailParamSchema).saleId,
      userId,
    ),
  );
  withTenantPos(router, "get", `${prefix}/tenant/pos/sales/:saleId`, async (userId, context) =>
    posController.getFaturaDetalhe(
      parseRouteParams<{ saleId: string }>(context.params, invoiceDetailParamSchema).saleId,
      userId,
    ),
  );
  withTenantPos(router, "get", `${prefix}/tenant/pos/faturas/:saleId/pdf`, async (userId, context) =>
    posController.getFaturaPdf(
      parseRouteParams<{ saleId: string }>(context.params, invoiceDetailParamSchema).saleId,
      userId,
    ),
  );
  withTenantPos(router, "get", `${prefix}/tenant/pos/sales/:saleId/pdf`, async (userId, context) =>
    posController.getFaturaPdf(
      parseRouteParams<{ saleId: string }>(context.params, invoiceDetailParamSchema).saleId,
      userId,
    ),
  );
  withTenantPos(router, "get", `${prefix}/tenant/pos/faturas/:saleId/print`, async (userId, context) =>
    posController.getFaturaPrint(
      parseRouteParams<{ saleId: string }>(context.params, invoiceDetailParamSchema).saleId,
      userId,
    ),
  );
  withTenantPos(router, "get", `${prefix}/tenant/pos/sales/:saleId/print`, async (userId, context) =>
    posController.getFaturaPrint(
      parseRouteParams<{ saleId: string }>(context.params, invoiceDetailParamSchema).saleId,
      userId,
    ),
  ...
  withTenantPos(router, "post", `${prefix}/tenant/pos/sales/:saleId/cancel`, async (userId, context: RouteContext) =>
    posController.anularFatura(
      context.req,
      userId,
      parseRouteParams<{ saleId: string }>(context.params, saleIdParamSchema).saleId,
    ),
  );
  ...
  router.patch(
    ...
    auditMiddleware,
    async (context) => {
      const { itemId } = parseRouteParams<{ itemId: string }>(context.params, draftCartItemIdParamSchema);
