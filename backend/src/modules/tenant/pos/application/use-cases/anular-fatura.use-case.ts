...
import { revertFaturaItemLoteStock } from "../services/fatura-item-lote.service";
    ...
    return await prisma.$transaction(async (tx: any) => {
      ...
      for (const item of items) {
        if (item.produtoId && item.produto) {
          ...
          const lotBreakdown = await revertFaturaItemLoteStock({
            tx,
            faturaItemId: item.id,
            fallbackLoteId: item.loteId,
            fallbackQuantidade: qty,
          });

          for (const entry of lotBreakdown) {
            await tx.$queryRaw`SELECT id FROM lotes WHERE id = ${entry.loteId} FOR UPDATE`;
            const loteRow = await tx.lote.findUnique({
              where: { id: entry.loteId },
              select: { quantidadeAtual: true },
            });
            const estoqueAnteriorLote = Number(loteRow?.quantidadeAtual ?? 0);

            await tx.lote.update({
              where: { id: entry.loteId },
              data: {
                quantidadeAtual: { increment: entry.quantidade },
                version: { increment: 1 },
              },
            });
            ...
            await tx.estoqueMovimento.create({
              data: {
                ...
                estoqueFinal: estoqueAnteriorLote + entry.quantidade,
          ...
          const lotePrincipal = lotBreakdown[0]?.loteId ?? item.loteId;

          const produtoFlat = flattenProdutoForApi(
            item.produto as Record<string, unknown>,
          );

          if (produtoFlat.requiresPsychotropicBook && lotePrincipal) {
            await tx.livroPsicotropico.create({
              data: {
                produtoId: item.produtoId,
                loteId: lotePrincipal,
