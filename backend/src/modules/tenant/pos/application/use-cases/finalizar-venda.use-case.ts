...
import { EnsureInvoiceDocumentSnapshotUseCase } from "../../../../documents/application/use-cases/ensure-invoice-document-snapshot.use-case";
import { QueueInvoiceArtifactGenerationUseCase } from "../../../../documents/application/use-cases/queue-invoice-artifact-generation.use-case";
import { draftCartService } from "../services/draft-cart.service";
import {
  resolveProdutoPrecoVendaFromRow,
  resolveServicoPrecoFromRow,
} from "../services/pos-pricing.service";
import {
  persistFaturaItemLotes,
  type FaturaItemLoteWrite,
} from "../services/fatura-item-lote.service";
...
export class FinalizarVendaUseCase {
  private readonly ensureSnapshotUseCase = new EnsureInvoiceDocumentSnapshotUseCase();
  private readonly queueArtifactsUseCase = new QueueInvoiceArtifactGenerationUseCase();
  ...
  async execute(data: FinalizarVendaDTO) {
    ...
    try {
      ...
      const result = await prisma.$transaction(async (tx: any) => {
        ...
        let checkoutScopedKey: string | null = null;
        if (data.idempotencyKey) {
          ...
          const scopedKey = draftFatura
            ? `TERM-${data.terminalId}:DRAFT-${draftFatura.id.toString()}`
            : `TERM-${data.terminalId}:${data.idempotencyKey}`;
          checkoutScopedKey = scopedKey;
        ...
        for (const item of data.items) {
          if (item.tipo === "produto") {
            ...
            const precoFinal = resolveProdutoPrecoVendaFromRow(produtoRow);
            ...
            const lotesUtilizados: FaturaItemLoteWrite[] = [];
            let ordemFefo = 0;
            ...
            checkoutLines.push({
              ...
              lotBreakdown: lotesUtilizados.map((entry) => ({
            ...
            faturaItems.push({
              produtoId,
              loteId: lotesUtilizados[0]?.loteId ?? null,
            ...
            const precoFinal = resolveServicoPrecoFromRow(servico);
            const fiscalCalc = FiscalCalculatorUtil.calcularIVA({
              quantidade: item.quantidade,
              precoUnitario: precoFinal,
              taxRule: taxRuleSnapshot,
        ...
        const fatura = await tx.fatura.create({
          data: {
            ...
            terminalId: BigInt(data.terminalId),
            userId: BigInt(data.userId),
            idempotencyKey: checkoutScopedKey,
        ...
        for (const itemData of faturaItems) {
          ...
          if (lotesUtilizados?.length) {
            await persistFaturaItemLotes(tx, faturaItem.id, lotesUtilizados);
          }
      ...
      try {
        ...
        const snapshot = await this.ensureSnapshotUseCase.execute(result.faturaId, data.userId);
        await this.queueArtifactsUseCase.execute({
        ...
        console.error("Falha ao preparar artefatos da fatura:", documentError);
      }
